...
-- =====================================================
 CREATE TABLE IF NOT EXISTS tbl_email_log (
     ...
     log_template_id INT,
     log_to VARCHAR(1000) NOT NULL,
     log_cc VARCHAR(1000),
     log_bcc VARCHAR(1000),
     log_subject VARCHAR(500) NOT NULL,
     log_body TEXT,
     log_sent_date DATETIME NOT NULL,
     log_status VARCHAR(20) NOT NULL DEFAULT 'PENDING',
     log_error_message TEXT,
     log_retry_count INT DEFAULT 0,
     log_sent_by INT, -- emp_number, NO FK
     log_reference_type VARCHAR(50),
     log_reference_id VARCHAR(100),
     ...
     INDEX idx_log_reference (log_reference_type, log_reference_id),

     CONSTRAINT fk_email_log_template
         FOREIGN KEY (log_template_id)
         REFERENCES tbl_email_template (template_id)
         ON DELETE SET NULL
 ) ENGINE=InnoDB
 DEFAULT CHARSET=utf8mb4
 COLLATE=utf8mb4_unicode_ci
COMMENT = 'Log of all emails sent by the system';
