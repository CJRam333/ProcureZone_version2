...
public interface IssueNoteRepository extends JpaRepository<IssueNote, Integer> {
        ...
        /**
         * Get the issue_note_no of the most recently created note (by issue_note_id DESC).
         * Mirrors the indent generator (legacy getIndentNo1 equivalent): the legacy issue note
         * numbers are a continuous 13-digit numeric sequence (e.g. 1100202000988) and new notes
         * must continue it, not switch to an IN/YYYY/NNNNN format.
         */
        @Query("SELECT i.issueNoteNumber FROM IssueNote i ORDER BY i.id DESC")
        java.util.List<String> findLatestIssueNoteNumbers(Pageable pageable);

        /**
         * MAX across all purely numeric issue_note_no values — fallback when the most recent
         * record carries a non-numeric number (e.g. IN/2026/00001 mis-generated by an earlier
         * deployment, which must be ignored so the sequence resumes from the real max).
         */
        @Query(value = "SELECT MAX(CAST(issue_note_no AS UNSIGNED)) FROM tbl_issue_note WHERE issue_note_no REGEXP '^[0-9]+$'", nativeQuery = true)
        Long findMaxNumericIssueNoteNumber();
