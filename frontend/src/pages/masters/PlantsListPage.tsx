...
import { FaPlus, FaEdit, FaTrash, FaSearch, FaSyncAlt, FaIndustry, FaCubes } from 'react-icons/fa';
import { toast } from 'react-toastify';
import { PageHeader, DataTable, ConfirmDialog } from '../../components/common';
import { plantsApi, getErrorMessage } from '../../api';
import type { Plant } from '../../api';
import CompanyPlantMaterialPage from '../mappings/CompanyPlantMaterialPage';
...
const PlantsListPage: React.FC = () => {
  ...
  return (
    <div>
      <PageHeader
        title="Plants"
        subtitle="Manage plant/facility master data and material mappings"
        breadcrumbs={[{ label: 'Dashboard', path: '/dashboard' }, { label: 'Masters', path: '/masters' }, { label: 'Plants' }]}
        actions={
          activeTab === 'list' ? (
            <Button variant="primary" onClick={() => navigate('/masters/plants/new')}>
              <FaPlus className="me-2" />Add Plant
            </Button>
          ) : undefined
        }
      />
      ...
      <Tabs
        ...
        id="plants-tabs"
      >
        <Tab eventKey="list" title={<><FaIndustry className="me-2" />Plants</>}>
          <Card>
            <Card.Body>
              <Row className="mb-3">
                <Col md={6}>
        ...
        <Tab eventKey="material-mapping" title={<><FaCubes className="me-2" />Material Mapping</>}>
          <CompanyPlantMaterialPage embedded />
        </Tab>
      </Tabs>
