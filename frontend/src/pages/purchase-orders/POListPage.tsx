...
const POListPage: React.FC = () => {
  ...
  const statusOptions = [
    ...
    { value: '0', label: 'Draft' },
    { value: '1', label: 'Pending Confirmation' },
  ...
  const columns = [
      ...
      render: (row: any) => (
        <div>
          ...
          <div className="small text-muted">
            {formatDate(row.createdAt || row.poDate)}
          </div>
        </div>
      ),
    },
    ...
    },
    {
      key: 'deliveryDate',
      label: 'Delivery Date',
      render: (row: any) => formatDate(row.deliveryDate),
    },
    {
      key: 'status',
      label: 'Status',
    ...
    },
    {
      ...
      render: (row: any) => (
        <div className="d-flex gap-1">
          ...
          {row.status === 'DRAFT' && hasAnyRole(['ADMIN', 'PO_CREATOR']) && (
            <Button
