# ProcureZone Backend — Complete End-to-End Project Brief

> **Purpose:** This document provides a complete, self-contained reference of the ProcureZone backend project. Feed this to any AI assistant (ChatGPT, Claude, etc.) so it has full context to help you track progress, debug, and extend the system.
>
...
| Field | Value |
|---|---|
| **Project Name** | ProcureZone Backend |
| **Owner** | NSL India Pvt Ltd |
| **Type** | Enterprise Procurement & Manufacturing Operations System |
| **Status** | 100% Complete (225 endpoints) |
| **Tech Stack** | Spring Boot 3.2.5, Java 21, MySQL 8.0, JWT Auth, Flyway, Quartz |
...
| Migration | Purpose |
|---|---|
| `V0__baseline.sql` | Initial schema baseline |
| `V20__create_email_tables.sql` | Email template/log tables |
| `V21__create_email_tables.sql` | Email table fixes |
| `V22__create_inventory_tables.sql` | Inventory & transaction tables |
| `V25__create_company_location_material_mapping.sql` | Location-material mapping |
| `V27__create_employee_reporting_hierarchy.sql` | Reporting hierarchy table |
| `V28__create_company_plant_material_table.sql` | Plant-material mapping |
| `V29__seed_indent_and_po_data.sql` | Seed data for testing |
| `V30__create_sap_integration_tables.sql` | SAP import tables |
| `V31__fix_email_tables_fk_and_schema_alignment.sql` | Schema alignment fixes |
...
| **Controllers** | 35 | REST API controllers |
| **Services** | 51 | Business logic |
| **Repositories** | 49 | JPA data access |
| **Entities/Domain** | 7+ | JPA entities |
| **DTOs** | 24+ | Request/response objects |
| **API Endpoints** | 225 | Complete REST API |
| **Database Tables** | 56 | MySQL schema |
| **Flyway Migrations** | 10 | Schema evolution |
| **User Roles** | 6 | RBAC roles |
| **Test Users** | 3 | Pre-seeded credentials |

---

> **Instructions for ChatGPT:** When helping with this project, remember that:
> 1. This is a **Spring Boot 3.2.5** project using **Java 21**
> 2. Database DDL is managed by **Flyway**, not Hibernate (`ddl-auto: none`)
> 3. Authentication is JWT-based with `JwtAuthenticationFilter`
> 4. Every module follows the **Controller → Service → Repository** pattern
> 5. There are **two sets** of PO tables and vendor tables (legacy + enhanced)
> 6. Status fields use `1/0` convention (Active/Inactive)
> 7. The database is called `seeds_indent` on MySQL
> 8. All endpoints require JWT except `/api/v1/auth/**`
> 9. The project is reportedly **100% complete** with 225 endpoints
> 10. SAP integration runs as a scheduled job at 1:30 AM daily
